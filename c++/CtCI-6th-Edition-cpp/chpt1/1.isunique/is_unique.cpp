...
bool isUniqueChars(const string &str){
		...
		for (int i = 0; i < str.length(); i++){
			...
			if (char_set[val]){
				return false;
			}                    //the order of reading is not necessarily equal to the order of process
			char_set[val] = true;//unique character will be envalued true here, instead of going into if loop
		}
		return true;
}
...
int main(){
		vector<string> words = {"abcdea", "hello", "apple", "kite", "padle"};
		for (auto word : words)
		{
